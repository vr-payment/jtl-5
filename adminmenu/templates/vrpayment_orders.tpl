...
{if $orders|@count > 0 && $orders}
    ...
    <form method="get" name="vrpayment-orders" action="{$currentUrl}">
        <div class="form-row">
            <!-- Label for the search input -->
            <label class="col-sm-auto col-form-label" for="orderSearch">{$translations.jtl_vrpayment_search}:</label>
    ...
    <form method="post" name="vrpayment-orders" action="{$postUrl}">
        {$jtl_token}
        <div class="table-responsive">
            <table class="table table-striped orders-table">
                <thead>
                <tr>
                    <th>{$translations.jtl_vrpayment_order_number}</th>
                    <th>{$translations.jtl_vrpayment_customer}</th>
                    <th>{$translations.jtl_vrpayment_payment_method}</th>
                    <th>{$translations.jtl_vrpayment_order_status}</th>
                    <th>{$translations.jtl_vrpayment_amount}</th>
                    <th></th>
                </tr>
                </thead>

                <div class="vrpayment-loader text-center" style="display: none">
...
{else}
    <div class="alert alert-info">
        ...
        {$translations.jtl_vrpayment_there_are_no_orders}
    </div>
{/if}
