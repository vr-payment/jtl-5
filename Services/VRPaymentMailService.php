...
/**
 * Service for sending emails related to VRPayment.
 */
class VRPaymentMailService {
    ...
    /**
     * Constructor for VRPaymentAbstractMailTemplate.
     *
    ...
    protected function canSendEmail(\stdClass $data, string $template): bool {
        if ($template == 'authorization') {
            $sendEmail = $this->pluginConfig[VRPaymentHelper::SEND_AUTHORIZATION_EMAIL] ?? null;

            return $sendEmail === 'YES' && !empty($data->tkunde->cMail);
        }
        elseif ($template == 'fulfill') {
            $sendEmail = $this->pluginConfig[VRPaymentHelper::SEND_FULFILL_EMAIL] ?? null;
