...
class VRPaymentTransactionService
{
     ...
     *      The email service.
     *
     * @see \Plugin\jtl_vrpayment\Services\VRPaymentMailService
     */
    protected $mailService;

    public function __construct(ApiClient $apiClient, $plugin)
    {
        $config = VRPaymentHelper::getConfigByID($plugin->getId());
        $spaceId = $config[VRPaymentHelper::SPACE_ID];

        $this->plugin = $plugin;
        $this->apiClient = $apiClient;
        $this->spaceId = $spaceId;
        $this->spaceViewId = $config[VRPaymentHelper::SPACE_VIEW_ID];

        $mailer = Shop::Container()->get(Mailer::class);
        $mail = new Mail();
        $db = Shop::Container()->getDB();
        $this->mailService = new VRPaymentMailService($mailer, $mail, $db, $config);
    }
    ...
    public function createTransaction(Bestellung $order): Transaction
    {
        ...
        $transactionPayload->setLanguage(VRPaymentHelper::getLanguageString());
        ...
        $successUrl = Shop::getURL() . '/' . VRPaymentHelper::PLUGIN_CUSTOM_PAGES['thank-you-page'][$_SESSION['cISOSprache']];
    ...
    public function confirmTransaction(Transaction $pendingTransaction): void
    {
        ...
        try {
            ...
            if ($createOrderAfterPayment === 1 && !empty($orderId)) {
                ...
                $_SESSION['vrpn_rollback_done_order_id'] = (int)$orderId;
            }
            $this->updateLocalVRPaymentTransaction((string)$transactionId, TransactionState::FAILED);
            throw $e;
        }

        if ($createOrderAfterPayment === 1) {
            $this->updateLocalVRPaymentTransaction((string)$transactionId);
        }
    }
    ...
    public function updateTransaction(int $transactionId)
    {
        ...
        if (empty($transaction) || empty($transaction->getVersion()) || $transaction->getState() !== TransactionState::PENDING) {
          ...
          $translations = VRPaymentHelper::getTranslations($this->plugin->getLocalization(), [
            ...
            Alert::TYPE_ERROR,
            $translations['jtl_vrpayment_transaction_timeout'],
    ...
    public function getTransactionPaymentMethod(int $transactionId, string $spaceId)
    {
        ...
        foreach ($possiblePaymentMethods as $possiblePaymentMethod) {
            if (VRPaymentHelper::PAYMENT_METHOD_PREFIX . '_' . $possiblePaymentMethod->getId() === $chosenPaymentMethod) {
     ...
     */
    public function getLocalVRPaymentTransactionById(string $transactionId): ?\stdClass
    {
        return Shop::Container()->getDB()->getSingleObject(
            'SELECT * FROM vrpayment_transactions WHERE transaction_id = :transaction_id ORDER BY id DESC LIMIT 1',
    ...
    public function createLocalVRPaymentTransaction(string $transactionId, array $orderData, $state = TransactionState::PENDING): void
    {
        ...
        Shop::Container()->getDB()->insert('vrpayment_transactions', $newTransaction);
    }
     ...
     * @return void
     */
    public function updateLocalVRPaymentTransaction(string $transactionId, $state = null, $orderId = null): void
    {
        if ($state === null) {
    ...
    public function cancelOrderOnce(int $orderId): bool {
        ...
        if ((int)$order->cStatus === \BESTELLUNG_STATUS_STORNO) {
            VRPaymentHelper::log("cancelOrderOnce: Order $orderId is already cancelled.");
            return false;
        }

        if (empty($order->kZahlungsart) ) {
            VRPaymentHelper::log("cancelOrderOnce: Order $orderId has no payment method.");
            return false;
        }

        if (VRPaymentHelper::checkDBColumnExists('vrpayment_transactions', 'cancellation_email_sent')) {
            $db = Shop::Container()->getDB();
            $affected = (int)$db->queryPrepared(
                "UPDATE vrpayment_transactions
            ...
            if ($affected < 1) {
                $row = $db->select('vrpayment_transactions', 'order_id', (string)$orderId);
                if (!empty($row)) {
                    VRPaymentHelper::log("cancelOrderOnce: The cancellation email for order $orderId has already been marked as sent.");
                    return false;
                }
            }
        }
        ...
        VRPaymentHelper::log("cancelOrderOnce: Cancelling order $orderId.");
    ...
    public function restoreStock(int $orderId) {
        ...
        foreach ($order->Positionen as $pos) {
            if ((int)$pos->nPosTyp === \C_WARENKORBPOS_TYP_ARTIKEL && (int)$pos->kArtikel > 0) {
                VRPaymentHelper::log("cancelOrderOnce: Restoring stock for Product $pos->kArtikel (Qty: $pos->nAnzahl)");
