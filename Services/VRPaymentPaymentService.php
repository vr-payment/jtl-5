...
class VRPaymentPaymentService
{
	...
	public function getPaymentMethodConfigurations(): ?array
	{
		...
		try {
			...
			$translations = VRPaymentHelper::getTranslations($plugin->getLocalization(), [
			  'jtl_vrpayment_cant_fetch_payment_methods',
			]);
			Shop::Container()->getAlertService()->addDanger(
			  $translations['jtl_vrpayment_cant_fetch_payment_methods'] . ' ' . $exception->getMessage(),
			  'getPaymentMethodConfigurations'
			);
			return [];
		}
	...
	public function installPaymentMethod(string $slug, array $data, int $sortIndex): ?int
	{
		...
		$method->nActive = 1;
		$method->cAnbieter = 'VRPayment';
		$method->cTSCode = \strtoupper(VRPaymentHelper::slugify($data['module']));
	...
	public function getInstalledPaymentMethods(int $status = self::STATUS_ENABLED): array
	{
		...
		$filteredObjects = array_filter($objects, function ($object) {
			return stripos($object->cAnbieter, 'VRPayment') !== false;
		});
	...
	public function syncPaymentMethods()
	{
		...
		foreach ($paymentMethods as $paymentMethod) {
			$slug = VRPaymentHelper::PAYMENT_METHOD_PREFIX . '_' . $paymentMethod->getId();
		...
		$plugin = PluginHelper::getLoaderByPluginID($this->pluginId)->init($this->pluginId);
		$translations = VRPaymentHelper::getTranslations($plugin->getLocalization(), [
		  'jtl_vrpayment_payment_methods_were_synchronised',
		]);
		Shop::Container()->getAlertService()->addSuccess(
		  $translations['jtl_vrpayment_payment_methods_were_synchronised'],
		  'syncPaymentMethods'
		);
	}
}
